...
from baykeshop.models.sites import BaykePermission, BaykeMenu
    ...
    def _build_menus(self, request):
        ...
        for menu in menus_queryset:
            ...
            for perm in menu.baykepermission_set.filter(is_show=True):
                ...
                # 自定义菜单名称
                bayke_perm = BaykePermission.objects.filter(permission__codename=f"view_{model._meta.model_name.lower()}")
                name = bayke_perm.first().name if bayke_perm.exists() else capfirst(model._meta.verbose_name_plural)
